...
class SortedSetLogTest {
    ...
    void creation() {
        ...
        Assertions.assertThrows(IllegalArgumentException.class,
                () -> new SortedSetLog<>(null, new IntegerIndexFactory(), new TreeSet<>()));

        Assertions.assertThrows(IllegalArgumentException.class,
                () -> new SortedSetLog<>(null, new TreeSet<>()));

        Assertions.assertThrows(IllegalArgumentException.class,
                () -> new SortedSetLog<>(new IntegerIndexFactory(), null));
    }
    ...
    @Test
    void hashCodeEquals() {
        Log<Integer, String> log1 = new SortedSetLog<>(new IntegerIndexFactory(), new TreeSet<>());
        Log<Integer, String> log2 = new SortedSetLog<>(new IntegerIndexFactory(), new TreeSet<>());

        assertThat(log1, is(not(log2)));

        UUID id = UUID.randomUUID();
        log1 = new SortedSetLog<>(id, new IntegerIndexFactory(), new TreeSet<>());
        log2 = new SortedSetLog<>(id, new IntegerIndexFactory(), new TreeSet<>());
