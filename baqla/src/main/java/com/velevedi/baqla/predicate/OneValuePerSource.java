...
/**
 * Entries in a Log could be filtered out. This predicate selects only one value per source on an Entry.
 *
 * @param <I> type of the index of an entry
 * @param <V> type of a value of an entry
 */
public class OneValuePerSource<I extends Comparable<? super I>, V> implements Predicate<Log.Entry<I, V>> {
    ...
    public boolean test(Log.Entry<I, V> entry) {
        ...
        if (selectedSources.contains(entry.source())) {
            return false;
        }
